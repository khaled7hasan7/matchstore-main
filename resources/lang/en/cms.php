<?php

return [
    'auth' => [
        'velstore' => 'MatchStore',
        'login' => 'Login',
        'email' => 'Email Address',
        'password' => 'Password',
        'remember_me' => 'Remember Me',
        'forgot_password' => 'Forgot Your Password?',
        'welcome_back' => 'Welcome Back!',
        'admin_portal_desc' => 'Access your admin dashboard to manage your store',
        'enter_credentials' => 'Enter your credentials to continue',
    ],

    'profile' => [
        'profile' => 'profile',
        'setting' => 'setting',
        'logout' => 'logout',
    ],

    'sidebar' => [
        'search_placeholder' => 'Search...',
        'dashboard' => 'Dashboard',
        'logo' => 'Logo',

        'categories' => [
            'title' => 'Categories',
            'add_new' => 'Add New',
            'list' => 'List',
        ],
        'brands' => [
            'title' => 'Brands',
            'add_new' => 'Add New',
            'list' => 'List',
        ],
        'products' => [
            'title' => 'Products',
            'add_new' => 'Add New',
            'list' => 'List',
        ],
        'attributes' => [
            'title' => 'Attributes',
            'add_new' => 'Add New',
            'list' => 'List',
        ],
        'customers' => [
            'title' => 'Customers',
            'list' => 'List',
        ],
        'vendors' => [
            'title' => 'Vendors',
            'add_new' => 'Add New',
            'list' => 'List',
        ],
        'product_reviews' => [
            'title' => 'Product Reviews',
            'list' => 'List',
        ],
        'banners' => [
            'title' => 'Banners',
            'add_new' => 'Add New',
            'list' => 'List',
        ],
        'promo_cards' => [
            'title' => 'Promo Cards',
            'add_new' => 'Add New',
            'list' => 'List',
        ],
        'coupons' => [
            'title' => 'Coupons',
            'add_new' => 'Add New',
            'list' => 'List',
        ],
        'menu' => [
            'title' => 'Menu',
            'add_new' => 'Add New',
            'list' => 'List',
        ],
        'menu_items' => [
            'title' => 'Menu Items',
            'add_new' => 'Add New',
            'list' => 'List',
        ],
        'social_media_links' => [
            'title' => 'Social Media Links',
            'add_new' => 'Add New',
            'list' => 'List',
        ],
        'site_settings' => [
            'title' => 'Site Settings',
            'manage' => 'Manage Settings',
        ],
        'orders' => [
            'title' => 'Orders',
            'all_orders' => 'All Orders',
            'pending_orders' => 'Pending Orders',
            'completed_orders' => 'Completed Orders',
            'list' => 'List',
        ],
        'pages' => [
            'title' => 'Pages',
            'add_new' => 'Add New',
            'list' => 'List',
        ],
        'payments' => [
            'title' => 'Payments',
            'list' => 'List',
        ],

        'refunds' => [
            'title' => 'Refunds',
            'list' => 'List',
        ],

        'payment_gateways' => [
            'title' => 'Payment Gateways',
            'list' => 'List',
        ],

        // General sidebar labels
        'role' => 'Role',
        'admin' => 'Admin',
        'joined' => 'Joined',
        'leave_blank' => 'Leave blank if you don\'t want to change password',
        'min_8_chars' => 'Minimum 8 characters',
        'password_strength' => 'Password Strength',
        'cancel' => 'Cancel',
    ],

    'profile' => [
        'title' => 'Edit Profile',
        'setting' => 'Manage your profile settings and password',
        'choose_file' => 'Choose File',
        'name' => 'Name',
        'email' => 'Email',
        'phone' => 'Phone',
        'personal_info' => 'Personal Information',
        'security' => 'Security Settings',
        'current_password' => 'Current Password',
        'new_password' => 'New Password',
        'confirm_new_password' => 'Confirm New Password',
        'save' => 'Save Changes',
        'profile_updated' => 'Profile updated successfully!',
        'success' => 'Success',
    ],

    'payment_gateways' => [

        // Page Titles
        'title' => 'Payment Gateways',
        'subtitle' => 'Manage payment gateway configurations',
        'edit_title' => 'Edit Payment Gateway',
        'edit_subtitle' => 'Update payment gateway settings',
        'back' => 'Back',

        // Table Headings
        'id' => 'ID',
        'name' => 'Name',
        'code' => 'Code',
        'status' => 'Status',
        'action' => 'Action',

        // Status Labels
        'active' => 'Active',
        'inactive' => 'Inactive',

        // Delete Modal
        'delete_confirm' => 'Confirm Delete',
        'delete_message' => 'Are you sure you want to delete this payment gateway?',
        'cancel' => 'Cancel',
        'delete' => 'Delete',

        // Alerts / Notifications
        'success' => 'Success',
        'deleted' => 'Deleted',
        'error_delete' => 'Error deleting payment gateway!',

        // Form Labels
        'gateway_name' => 'Gateway Name',
        'description' => 'Description',
        'active_label' => 'Active',
        'configurations' => 'Gateway Configurations',
        'no_configurations' => 'No configurations found',
        'key_name' => 'Key Name',
        'key_value' => 'Key Value',
        'environment' => 'Environment',
        'sandbox' => 'Sandbox',
        'production' => 'Production',
        'encrypted' => 'Encrypted',
        'unique' => 'unique',
        'update_button' => 'Update',

        // Fallback
        'not_available' => 'N/A',
    ],

    'refunds' => [

        // Page Titles
        'title' => 'Refunds',
        'subtitle' => 'Manage refund requests',
        'details_title' => 'Refund Details',
        'details_subtitle' => 'View refund request details',

        // Table Headings
        'id' => 'ID',
        'payment' => 'Payment',
        'amount' => 'Amount',
        'status' => 'Status',
        'reason' => 'Reason',
        'action' => 'Action',

        // Status Labels
        'completed' => 'Completed',
        'pending' => 'Pending',
        'failed' => 'Failed',

        // Delete Modal
        'delete_confirm' => 'Confirm Delete',
        'delete_message' => 'Are you sure you want to delete this refund?',
        'cancel' => 'Cancel',
        'delete' => 'Delete',

        // Alerts / Notifications
        'success' => 'Success',
        'deleted' => 'Deleted',
        'delete_error' => 'Error deleting refund!',

        // Refund Details Page
        'created_at' => 'Created At',
        'updated_at' => 'Updated At',
        'back' => 'Back to Refunds',

        // Fallback
        'not_available' => 'N/A',
    ],

    'payments' => [

        // Page Titles
        'title' => 'Payments',
        'subtitle' => 'Manage payment transactions',
        'details_title' => 'Payment Details',
        'details_subtitle' => 'View payment transaction details',

        // Table Headings
        'id' => 'ID',
        'order' => 'Order',
        'user' => 'User',
        'gateway' => 'Gateway',
        'amount' => 'Amount',
        'status' => 'Status',
        'transaction' => 'Transaction',
        'action' => 'Action',

        // Status Labels
        'completed' => 'Completed',
        'pending' => 'Pending',
        'failed' => 'Failed',

        // Delete Modal
        'delete_confirm' => 'Confirm Delete',
        'delete_message' => 'Are you sure you want to delete this payment?',
        'cancel' => 'Cancel',
        'delete' => 'Delete',

        // Alerts / Notifications
        'success' => 'Success',
        'deleted' => 'Deleted',
        'error_delete' => 'Error deleting payment!',

        // Payment Details Page
        'transaction_id' => 'Transaction ID',
        'created_at' => 'Created At',
        'back' => 'Back to Payments',

        // Fallback
        'not_available' => 'N/A',
    ],

    'pages' => [
        // General
        'title' => 'Pages',
        'subtitle' => 'Manage website pages and content',
        'add_page' => 'Add Page',
        'choose_file' => 'Choose File',

        // Create Page
        'create' => 'Create Page',
        'form_title' => 'Title (:code)',
        'form_content' => 'Content (:code)',
        'form_image' => 'Image (:code)',
        'form_save' => 'Save',

        // Edit Page
        'edit' => 'Edit Page',
        'form_update' => 'Update',

        // Pages Table
        'table_title' => 'Title',
        'table_slug' => 'Slug',
        'table_status' => 'Status',
        'table_actions' => 'Actions',

        // Delete Modal
        'delete_modal_title' => 'Confirm Deletion',
        'delete_modal_text' => 'Are you sure you want to delete this page?',
        'delete_modal_cancel' => 'Cancel',
        'delete_modal_delete' => 'Delete',

        // Toastr messages
        'toastr_success' => 'Success',
        'toastr_error' => 'Error deleting page',
    ],

    'customers' => [
        'customer_list' => 'Customer List',
        'subtitle' => 'Manage customer accounts',
        'create_title' => 'Add Customer',
        'edit_title' => 'Edit Customer',
        'password' => 'Password',
        'password_hint_edit' => 'Leave blank to keep the current password',
        'save' => 'Save',

        // Table columns
        'id' => 'Id',
        'name' => 'Name',
        'email' => 'Email',
        'phone' => 'Phone',
        'address' => 'Address',
        'status' => 'Status',
        'actions' => 'Actions',

        // Status labels
        'active' => 'Active',
        'inactive' => 'Inactive',

        // Modal
        'confirm_delete_title' => 'Confirm Delete',
        'confirm_delete_message' => 'Are you sure you want to delete this customer?',
        'cancel_button' => 'Cancel',
        'delete_button' => 'Delete',

        // Toastr messages
        'success_title' => 'Success',
        'deleted_title' => 'Deleted',
        'delete_success_message' => 'Customer deleted successfully!',
        'delete_error_message' => 'Error deleting customer!',
    ],

    'vendors' => [
        'title_list' => 'Vendor List',
        'subtitle' => 'Manage vendor accounts',
        'add_new' => 'Add Vendor',
        'create_subtitle' => 'Register a new vendor account',
        'back' => 'Back',
        'min_8_chars' => 'Minimum 8 characters',
        'id' => 'Id',
        'name' => 'Name',
        'email' => 'Email',
        'phone' => 'Phone',
        'status' => 'Status',
        'actions' => 'Actions',
        'register_new_vendor' => 'Register New Vendor',
        'vendor_name' => 'Vendor Name',
        'vendor_email' => 'Vendor Email',
        'phone_optional' => 'Phone (Optional)',
        'password' => 'Password',
        'confirm_password' => 'Confirm Password',
        'status' => 'Status',
        'active' => 'Active',
        'inactive' => 'Inactive',
        'banned' => 'Banned',
        'register_button' => 'Register Vendor',
        'cancel_button' => 'Cancel',

        'active' => 'Active',
        'inactive' => 'Inactive',

        'modal_confirm_delete_title' => 'Confirm Delete',
        'modal_confirm_delete_body' => 'Are you sure you want to delete this vendor?',
        'delete' => 'Delete',
        'cancel' => 'Cancel',

        'success' => 'Success',
        'error_delete' => 'Error deleting vendor! Please try again.',
    ],

    'languages' => [
        'english' => 'English',
        'spanish' => 'Spanish',
        'french' => 'French',
        'arabic' => 'Arabic',
        'german' => 'German',
        'persian' => 'Persian (Farsi)',
        'hindi' => 'Hindi',
        'indonesian' => 'Indonesian',
        'italian' => 'Italian',
        'japanese' => 'Japanese',
        'korean' => 'Korean',
        'dutch' => 'Dutch',
        'polish' => 'Polish',
        'portuguese' => 'Portuguese',
        'russian' => 'Russian',
        'thai' => 'Thai',
        'turkish' => 'Turkish',
        'vietnamese' => 'Vietnamese',
        'chinese' => 'Chinese',

        'language_change' => 'Language Change',
        'change_language' => 'Change Language',
        'confirm_language_change' => 'Are you sure you want to change the language?',
        'cancel' => 'Cancel',
        'yes_change' => 'Yes, Change',
    ],

    'categories' => [
        'heading' => 'Categories',
        'subtitle' => 'Manage product categories',
        'add_category' => 'Add Category',
        'image' => 'Image',
        'choose_file' => 'Choose File',
        'id' => 'Id',
        'name' => 'Category Name',
        'description' => 'Description',
        'action' => 'Action',
        'edit' => 'edit',
        'delete' => 'delete',
        'button' => 'Save',
        'add_new' => 'Add New',
        'status' => 'Status',
        'created' => 'Category created successfully!',
        'updated' => 'Category updated successfully!',
        'deleted' => 'Category deleted successfully!',
        'status_updated' => 'Category status updated successfully!',
        'massage_confirm' => 'Confirm Deletion',
        'confirm_delete' => 'Are you sure you want to delete this category?',
        'massage_cancel' => 'Cancel',
        'massage_delete' => 'Delete',
        'success' => 'Success',
    ],

    'datatables' => [
        'sEmptyTable' => 'No data available in table',
        'sInfo' => 'Showing _START_ to _END_ of _TOTAL_ entries',
        'sInfoEmpty' => 'Showing 0 to 0 of 0 entries',
        'sInfoFiltered' => '(filtered from _MAX_ total entries)',
        'sLengthMenu' => 'Show _MENU_ entries',
        'sLoadingRecords' => 'Loading...',
        'sProcessing' => 'Processing...',
        'sSearch' => 'Search:',
        'sZeroRecords' => 'No matching records found',
        'oPaginate' => [
            'sFirst' => 'First',
            'sLast' => 'Last',
            'sNext' => 'Next',
            'sPrevious' => 'Previous',
        ],
    ],

    'products' => [

        // Titles
        'title_create' => 'Create Product',
        'title_edit' => 'Edit Product',
        'title_manage' => 'Manage Products',
        'manage_subtitle' => 'Manage your product catalog',
        'add_product' => 'Add Product',

        // Form Fields
        'vendor' => 'Vendor',
        'select_vendor' => 'Select Vendor',
        'product_name' => 'Product Name',
        'product_type' => 'Product Type',
        'description' => 'Description',
        'translations' => 'Translations',
        'translated_value' => 'Translated Value',
        'category' => 'Category',
        'brand' => 'Brand',
        'no_brand' => 'No Brand',
        'status' => 'Status',
        'price' => 'Price',
        'discount_price' => 'Discount Price',
        'sku' => 'SKU',
        'barcode' => 'Barcode',
        'stock' => 'Stock',
        'weight' => 'Weight',
        'dimension' => 'Dimensions',
        'size' => 'Size',
        'color' => 'Color',
        'images' => 'Product Images',
        'is_primary' => 'Primary Variant',
        'variant_name_en' => 'Variant Name',
        'attributes' => 'Attributes',
        'attribute_values' => 'Attribute Values',
        'variants' => 'Variants',

        // Buttons
        'add_variant' => 'Add Variant',
        'remove_variant' => 'Remove Variant',
        'save_product' => 'Save Product',
        'update_product' => 'Update Product',
        'choose_images' => 'Choose Images',
        'choose_file' => 'Choose File',
        'remove' => 'Remove',

        // Messages
        'status_updated' => 'product status updated successfully!',
        'success_create' => 'Product created successfully!',
        'success_update' => 'Product updated successfully!',
        'success_delete' => 'Product deleted successfully!',
        'delete_confirmation' => 'Are you sure you want to delete this product?',
        'success' => 'Success',

        // Table Columns
        'id' => 'ID',
        'name' => 'Name',
        'type' => 'Type',
        'price_column' => 'Price',
        'status_column' => 'Status',
        'action' => 'Action',

        // Confirmation Dialog
        'confirm_delete' => 'Confirm Delete',
        'delete' => 'Delete',
        'cancel' => 'Cancel',
    ],

    'brands' => [
        'heading' => 'Brands',
        'subtitle' => 'Manage product brands',
        'add_brand' => 'Add Brand',
        'id' => 'ID',
        'name' => 'Name',
        'description' => 'Description',
        'logo' => 'Logo',
        'status' => 'Status',
        'action' => 'Action',
        'edit' => 'Edit',
        'delete' => 'Delete',
        'create' => 'Create',
        'update' => 'Update',
        'add_new' => 'Add New',
        'button' => 'Save',
        'form_title' => 'Create or Edit Brand',
        'file_upload' => 'Upload Logo',
        'no_logo' => 'No Logo',
        'choose_file' => 'Choose file',
        'image_preview' => 'Image Preview',
        'delete_confirmation' => 'Are you sure you want to delete this brand?',
        'brand_deleted' => 'Brand deleted successfully!',
        'error_delete' => 'Error deleting brand!',
        'created' => 'Brand created successfully!',
        'updated' => 'Brand updated successfully!',
        'deleted' => 'Brand deleted successfully!',
        'status_updated' => 'Brand status updated successfully!',
        'massage_confirm' => 'Confirm Deletion',
        'confirm_delete' => 'Are you sure you want to delete this brand?',
        'massage_cancel' => 'Cancel',
        'massage_delete' => 'Delete',
        'success' => 'Success',
    ],

    'coupons' => [
        'heading' => 'Coupons',
        'subtitle' => 'Manage discount coupons and promo codes',
        'create_subtitle' => 'Create a new discount coupon',
        'edit_subtitle' => 'Update coupon details',
        'create' => 'Create Coupon',
        'edit' => 'Edit Coupon',
        'id' => 'ID',
        'code' => 'Coupon Code',
        'discount' => 'Discount',
        'discount_percentage' => 'Discount Percentage',
        'discount_amount' => 'Discount Amount',
        'type' => 'Discount Type',
        'percentage' => 'Percentage',
        'fixed' => 'Fixed Amount',
        'expires_at' => 'Expiration Date',
        'status' => 'Status',
        'active' => 'Active',
        'expired' => 'Expired',
        'no_expiry' => 'No Expiration',
        'action' => 'Actions',
        'save' => 'Save Coupon',
        'update' => 'Update Coupon',
        'cancel' => 'Cancel',
        'no_coupons' => 'No coupons found.',
        'success_create' => 'Coupon created successfully!',
        'success_update' => 'Coupon updated successfully!',
        'success_delete' => 'Coupon deleted successfully!',
        'massage_confirm' => 'Confirm Deletion',
        'confirm_delete' => 'Are you sure you want to delete this coupon?',
        'massage_cancel' => 'Cancel',
        'massage_delete' => 'Delete',
        'code_help' => 'Enter a unique coupon code (e.g., SUMMER2024)',
        'discount_help_percentage' => 'Enter percentage (e.g., 20 for 20% off)',
        'discount_help_fixed' => 'Enter fixed amount (e.g., 10 for $10 off)',
        'expires_help' => 'Leave empty for no expiration',
        'expired_notice' => 'This coupon has expired and will not be available to customers.',
    ],

    'banners' => [
        'manage_subtitle' => 'Manage promotional banners',
        'add_banner' => 'Add Banner',
        'create_subtitle' => 'Create a new promotional banner',
        'button_back' => 'Back',
        'description' => 'Description',
        'choose_file' => 'Choose file',
        'all_banners' => 'All Banners',
        'id' => 'Id',
        'add_new' => 'Add New',
        'banner_type' => 'Banner Type',
        'promotion' => 'Promotion',
        'sale' => 'Sale',
        'seasonal' => 'Seasonal',
        'featured' => 'Featured',
        'announcement' => 'Announcement',
        'actions' => 'Actions',
        'edit' => 'Edit',
        'delete' => 'Delete',
        'no_image' => 'No image available',
        'delete_confirmation' => 'Are you sure you want to delete this banner?',
        'banner_deleted' => 'Banner deleted successfully!',
        'error_delete' => 'Error deleting banner!',
        'image_preview' => 'Image Preview',
        'create_banner' => 'Create Banner',
        'edit_banner' => 'Edit Banner Translations',
        'save' => 'Save',
        'languages' => 'Languages',
        'status' => 'Status',
        'image' => 'Image',
        'image_title' => 'Image Title',
        'title' => 'Title',
        'select_language' => 'Select Language',
        'file_upload' => 'Upload Image',
        'choose_file' => 'Choose File',
        'existing_image' => 'Existing Image',
        'banner_saved' => 'Banner saved successfully!',
        'banner_updated' => 'Banner updated successfully!',
        'form_title' => 'Create or Edit Banner',
        'form_description' => 'Please provide all the necessary information for the banner.',
        'back_to_list' => 'Back to Banner List',
        'created' => 'Banner created successfully!',
        'updated' => 'Banner updated successfully!',
        'deleted' => 'Banner deleted successfully!',
        'status_updated' => 'Banner status updated successfully!',
        'massage_confirm' => 'Confirm Deletion',
        'confirm_delete' => 'Are you sure you want to delete this banner?',
        'massage_cancel' => 'Cancel',
        'massage_delete' => 'Delete',
        'success' => 'Success',
    ],

    'promo_cards' => [
        'all_promo_cards' => 'All Promo Cards',
        'manage_subtitle' => 'Manage promotional cards for the homepage',
        'add_promo_card' => 'Add Promo Card',
        'create_promo_card' => 'Create Promo Card',
        'edit_promo_card' => 'Edit Promo Card',
        'create_subtitle' => 'Create a new promotional card',
        'edit_subtitle' => 'Edit promotional card details',
        'back_to_list' => 'Back to List',
        'id' => 'ID',
        'title' => 'Title',
        'image' => 'Image',
        'size' => 'Size',
        'size_large' => 'Large',
        'size_small' => 'Small',
        'order' => 'Display Order',
        'order_help' => 'Lower numbers appear first',
        'status' => 'Status',
        'active' => 'Active',
        'inactive' => 'Inactive',
        'actions' => 'Actions',
        'translations' => 'Translations',
        'badge_text' => 'Badge Text',
        'badge_text_help' => 'Optional small text shown as a badge',
        'button_text' => 'Button Text',
        'button_url' => 'Button URL',
        'button_url_help' => 'Link when button is clicked (e.g., /shop)',
        'image_url' => 'Image URL',
        'image_url_help' => 'Full URL to the promotional image',
        'no_image' => 'No Image',
        'no_promo_cards' => 'No promo cards found',
        'save' => 'Save Promo Card',
        'update' => 'Update Promo Card',
        'cancel' => 'Cancel',
        'delete' => 'Delete',
        'confirm_title' => 'Confirm Deletion',
        'confirm_delete' => 'Are you sure you want to delete this promo card?',
        'created' => 'Promo card created successfully!',
        'updated' => 'Promo card updated successfully!',
        'deleted' => 'Promo card deleted successfully!',
        'status_updated' => 'Promo card status updated successfully!',
        'success' => 'Success',
        'failed' => 'Failed',
        'error_updating' => 'Error updating promo card status!',
        'error_deleting' => 'Error deleting promo card!',
    ],

    'menus' => [
        'manage_subtitle' => 'Manage navigation menus',
        'all_menus' => 'All Menus',
        'id' => 'Id',
        'add_new' => 'Add New',
        'button_create' => 'Create',
        'button_update' => 'Update',
        'title' => 'Title',
        'edit' => 'Edit',
        'action' => 'Action',
        'delete' => 'Delete',
        'created_at' => 'Created At',
        'no_menus' => 'No menus available',
        'delete_confirmation' => 'Are you sure you want to delete this menu?',
        'menu_deleted' => 'Menu deleted successfully!',
        'error_delete' => 'Error deleting menu!',
        'create_menu' => 'Create Menu',
        'edit_menu' => 'Edit Menu',
        'save' => 'Save',
        'menu_title' => 'Menu Title',
        'form_title' => 'Create or Edit Menu',
        'form_description' => 'Please provide all the necessary information for the menu.',
        'back_to_list' => 'Back to Menu List',
        'created' => 'Menu created successfully!',
        'updated' => 'Menu updated successfully!',
        'deleted' => 'Menu deleted successfully!',
        'status_updated' => 'Menu status updated successfully!',
        'massage_confirm' => 'Confirm Deletion',
        'confirm_delete' => 'Are you sure you want to delete this menu?',
        'massage_cancel' => 'Cancel',
        'massage_delete' => 'Delete',
        'success' => 'Success',

    ],

    'menu_items' => [
        'subtitle' => 'Manage menu items and links',
        'heading' => 'All Menu Items',
        'id' => 'Id',
        'create' => 'Create Menu Item',
        'choose_an_option' => 'Choose an Option',
        'select_an_option' => 'Select an Option',
        'option1' => 'Option 1',
        'option2' => 'Option 2',
        'option3' => 'Option 3',
        'option4' => 'Option 4',
        'order_number' => 'Order Number',
        'parent_item' => 'Parent Item',
        'parent_none' => 'None',
        'edit' => 'Edit Menu Item',
        'update' => 'Update Menu Item',
        'delete' => 'Delete Menu Item',
        'title' => 'Title',
        'button' => 'Save',
        'update_button' => 'Update',
        'slug' => 'Slug',
        'order' => 'Order',
        'actions' => 'Actions',
        'add_new' => 'Add New',
        'submit' => 'Submit',
        'cancel' => 'Cancel',
        'no_title' => 'No Title',
        'select_menu' => 'Select a Menu',
        'select_parent_item' => 'Select Parent Item',
        'language' => 'Language',
        'select_language' => 'Select Language',
        'select_order' => 'Select Order',
        'success_message' => 'Menu Item has been successfully created!',
        'error_message' => 'There was an error creating the menu item.',
        'confirm_delete' => 'Are you sure you want to delete this menu item?',
        'update_success_message' => 'Menu Item has been successfully updated!',
        'update_error_message' => 'There was an error updating the menu item.',
        'created' => 'Menu item created successfully!',
        'updated' => 'Menu item updated successfully!',
        'deleted' => 'Menu item deleted successfully!',
        'status_updated' => 'Menu item status updated successfully!',
        'massage_confirm' => 'Confirm Deletion',
        'confirm_delete' => 'Are you sure you want to delete this menu item?',
        'massage_cancel' => 'Cancel',
        'massage_delete' => 'Delete',
        'success' => 'Success',

    ],

    'errors' => [
        'whoops' => 'Whoops!',
        'input_problem' => 'There were some problems with your input.',
        'validation_failed' => 'Validation failed! Please correct the errors and try again.',
        'csrf_token_invalid' => 'Invalid CSRF token. Please refresh the page and try again.',
        'not_found' => 'The requested item could not be found.',
        'unauthorized' => 'You are not authorized to perform this action.',
    ],

    'messages' => [
        'welcome' => 'Welcome to the Admin Panel!',
        'dashboard' => 'Dashboard',
        'settings' => 'Settings',
        'log_out' => 'Log Out',
        'profile' => 'Profile',
        'menu' => 'Menu',
        'home' => 'Home',
        'view_details' => 'View Details',
    ],

    'social_media_links' => [
        'type' => 'Social Media Type',
        'select_type' => 'Select Social Media Type',
        'types' => [
            'facebook' => 'Facebook',
            'instagram' => 'Instagram',
            'tiktok' => 'TikTok',
            'youtube' => 'YouTube',
            'x' => 'X',
        ],
        'id' => 'Id',
        'create' => 'Create Social Media Link',
        'edit' => 'Edit Social Media Link',
        'platform' => 'Platform Name',
        'link' => 'Social Media Link',
        'created' => 'Social media link created successfully!',
        'updated' => 'Social media link updated successfully!',
        'deleted' => 'Social media link deleted successfully!',
        'status_updated' => 'Social media link status updated successfully!',
        'massage_confirm' => 'Confirm Deletion',
        'confirm_delete' => 'Are you sure you want to delete this social media link?',
        'massage_cancel' => 'Cancel',
        'massage_delete' => 'Delete',
        'success' => 'Success',

        'translations' => [
            'platform_name' => 'Platform Name (translated)',
        ],
        'save' => 'Save',
        'update' => 'Update',
        'delete' => 'Delete',
        'no_links' => 'No Social Media Links available',
        'delete_confirmation' => 'Are you sure you want to delete this link?',
        'link_deleted' => 'Social Media Link deleted successfully!',
        'error_delete' => 'Error deleting link!',
        'create_link' => 'Create Social Media Link',
        'edit_link' => 'Edit Social Media Link',
        'form_title' => 'Create or Edit Social Media Link',
        'form_description' => 'Please provide all the necessary information for the social media link.',
        'back_to_list' => 'Back to Social Media Links List',
        'add_new' => 'Add New',
        'trans_name' => 'Translated Name',
        'delete' => 'Delete',
        'action' => 'Action',
    ],

    'orders' => [

        // Page Title
        'title' => 'Orders List',
        'subtitle' => 'Manage all customer orders',
        'all_orders' => 'All Orders',
        'pending' => 'Pending',
        'processing' => 'Processing',
        'completed' => 'Completed',
        'cancelled' => 'Cancelled',

        // Table Headings
        'id' => 'Order ID',
        'order_date' => 'Order Date',
        'status' => 'Status',
        'total_price' => 'Total Price',
        'action' => 'Actions',
        'customer' => 'Customer',

        // Delete Modal
        'delete_confirm_title' => 'Confirm Delete',
        'delete_confirm_message' => 'Are you sure you want to delete this order?',
        'delete_cancel' => 'Cancel',
        'delete_button' => 'Delete',

        // Toastr / Flash Messages
        'deleted_success' => 'Order deleted successfully.',
        'deleted_error' => 'Failed to delete the order.',
        'deleted' => 'Deleted',
        'success' => 'Success',
    ],

    'attributes' => [
        'title_create' => 'Create Attribute',
        'title_edit' => 'Edit Attribute',
        'title_manage' => 'Manage Attributes',
        'manage_subtitle' => 'Manage product attributes and variations',
        'add_attribute' => 'Add Attribute',

        'attribute_name' => 'Attribute Name',
        'attribute_values' => 'Attribute Values',
        'translations' => 'Translations',
        'translated_value' => 'Translated Value',

        'add_value' => 'Add Value',
        'remove_value' => 'Remove',
        'save_attribute' => 'Save Attribute',
        'update_attribute' => 'Update Attribute',
        'add_value_translation' => 'Add Value Translation',

        'success_create' => 'Attribute created successfully!',
        'success_update' => 'Attribute updated successfully!',
        'success_delete' => 'Attribute deleted successfully!',
        'delete_confirmation' => 'Are you sure you want to delete this attribute?',
        'success' => 'Success',

        'id' => 'ID',
        'name' => 'Name',
        'values' => 'Values',
        'action' => 'Action',

        'confirm_delete' => 'Confirm Delete',
        'delete' => 'Delete',
        'cancel' => 'Cancel',
    ],

    'product_reviews' => [
        'title_manage' => 'Product Reviews',
        'subtitle' => 'Manage product reviews and ratings',
        'details_subtitle' => 'View review details',

        'review_id' => 'Review ID',
        'customer_name' => 'Customer Name',
        'product_name' => 'Product Name',
        'rating' => 'Rating',
        'status' => 'Status',
        'actions' => 'Actions',
        'review_details' => 'Review Details',
        'review' => 'Review',
        'back_button' => 'Back to List',

        'confirm_delete' => 'Confirm Delete',
        'delete_message' => 'Are you sure you want to delete this product review?',
        'delete' => 'Delete',
        'cancel' => 'Cancel',

        'success_create' => 'Product review created successfully!',
        'success_update' => 'Product review updated successfully!',
        'success_delete' => 'Product review deleted successfully!',
        'error_delete' => 'Error deleting product review! Please try again.',

        'success' => 'Success',
        'error' => 'Error',
    ],

    'dashboard' => [
        'title' => 'Dashboard Overview',
        'subtitle' => 'Monitor your store\'s performance and manage key operations',
        'total_sales' => 'Total Sales',
        'today' => 'Today',
        'total_orders' => 'Total Orders',
        'completed' => 'Completed',
        'active_vendors' => 'Active Vendors',
        'all_active_accounts' => 'All active accounts',
        'total_customers' => 'Total Customers',
        'active_customer_base' => 'Active customer base',
        'recent_orders' => 'Recent Orders',
        'view_all' => 'View All',
        'order_id' => 'Order ID',
        'customer' => 'Customer',
        'date' => 'Date',
        'total' => 'Total',
        'status' => 'Status',
        'action' => 'Action',
        'no_recent_orders' => 'No recent orders found',
        'quick_actions' => 'Quick Actions',
        'add_new_product' => 'Add New Product',
        'add_new_product_desc' => 'Create a new product listing',
        'manage_orders' => 'Manage Orders',
        'manage_orders_desc' => 'View and process orders',
        'view_customers' => 'View Customers',
        'view_customers_desc' => 'Manage customer accounts',
        'create_coupon' => 'Create Coupon',
        'create_coupon_desc' => 'Add new discount codes',
        'site_settings' => 'Site Settings',
        'site_settings_desc' => 'Configure store settings',
    ],

    'site_settings' => [
        'title' => 'Site Settings',
        'edit_title' => 'Edit Site Settings',
        'edit_settings' => 'Edit Settings',
        'update_settings' => 'Update Settings',
        'setting' => 'Setting',
        'value' => 'Value',
        'not_set' => 'Not set',

        // Tabs
        'tab_general' => 'General',
        'tab_contact' => 'Contact',
        'tab_seo' => 'SEO',
        'tab_branding' => 'Branding',

        // General fields
        'site_name' => 'Site Name',
        'tagline' => 'Tagline',
        'tagline_help' => 'A short phrase describing your store',
        'default_currency' => 'Default Currency',
        'default_currency_help' => 'The default currency for first-time visitors',
        'default_language' => 'Default Language',
        'default_language_help' => 'The default language for first-time visitors',

        // Contact fields
        'contact_email' => 'Contact Email',
        'contact_phone' => 'Contact Phone (Primary)',
        'contact_phone_2' => 'Contact Phone (Secondary)',
        'contact_phone_2_help' => 'Optional second phone number',
        'address' => 'Address',

        // SEO fields
        'meta_title' => 'Meta Title',
        'meta_title_help' => 'Title shown in search engine results (50-60 characters recommended)',
        'meta_description' => 'Meta Description',
        'meta_description_help' => 'Description shown in search engine results (150-160 characters recommended)',
        'meta_keywords' => 'Meta Keywords',
        'meta_keywords_help' => 'Comma-separated keywords (e.g., ecommerce, online store, fashion)',
        'footer_text' => 'Footer Text',
        'footer_text_help' => 'Copyright text or additional footer information',

        // Branding fields
        'logo' => 'Logo',
        'logo_help' => 'Recommended: PNG, JPG, SVG, or WebP. Max size: 2MB',
        'current_logo' => 'Current Logo',
        'no_logo_uploaded' => 'No logo uploaded',
        'favicon' => 'Favicon',
        'favicon_help' => 'Recommended: PNG or ICO. Max size: 1MB. Ideal size: 32x32 or 16x16 pixels',
        'current_favicon' => 'Current Favicon',
        'no_favicon_uploaded' => 'No favicon uploaded',

        // Theme Tab
        'tab_theme' => 'Theme',
        'theme_presets' => 'Theme Presets',
        'theme_presets_help' => 'Choose a pre-designed color scheme for your store',
        'enable_custom_colors' => 'Enable Custom Colors',
        'enable_custom_colors_help' => 'Override preset theme with your own custom colors',
        'custom_colors' => 'Custom Colors',

        // Color Fields
        'primary_color' => 'Primary Brand Color',
        'primary_color_help' => 'Main brand color used for buttons, links, and accents',
        'primary_light_color' => 'Light Primary Color',
        'primary_light_color_help' => 'Lighter shade for hover states and secondary elements',
        'secondary_color' => 'Secondary/Accent Color',
        'secondary_color_help' => 'Used for section dividers and secondary elements',
        'background_color' => 'Background Color',
        'background_color_help' => 'Main page background color',
        'card_bg_color' => 'Card Background',
        'card_bg_color_help' => 'Background color for cards and product tiles',
        'text_color' => 'Text Color',
        'text_color_help' => 'Primary text color throughout the site',
        'border_color' => 'Border Color',
        'border_color_help' => 'Color for borders and dividing lines',

        // Preview
        'theme_preview' => 'Live Preview',
        'preview_header' => 'Preview Header',
        'update_theme' => 'Update Theme Settings',

        // Theme Preset Names
        'theme_classic_black' => 'Classic Black',
        'theme_purple_elegance' => 'Purple Elegance',
        'theme_ocean_blue' => 'Ocean Blue',
        'theme_forest_green' => 'Forest Green',
        'theme_sunset_orange' => 'Sunset Orange',
        'theme_modern_teal' => 'Modern Teal',
        'theme_match_systems' => 'Match Systems',
    ],

    'shipping_zones' => [
        'title' => 'Shipping Zones',
        'add_new_zone' => 'Add New Zone',
        'create_zone' => 'Create Shipping Zone',
        'edit_zone' => 'Edit Shipping Zone',

        // Table columns
        'id' => '#',
        'name' => 'Name',
        'countries' => 'Countries',
        'base_cost' => 'Base Cost',
        'per_kg_cost' => 'Per KG Cost',
        'status' => 'Status',
        'actions' => 'Actions',

        // Messages
        'failed_update_status' => 'Failed to update status',
        'confirm_delete' => 'Are you sure you want to delete this shipping zone?',
        'failed_delete' => 'Failed to delete shipping zone',
        'deleted_success' => 'Shipping zone deleted successfully',
    ],

    'order_details' => [
        'title' => 'Order #:id',
        'back_to_orders' => 'Back to Orders',

        // Sections
        'customer_information' => 'Customer Information',
        'shipping_address' => 'Shipping Address',
        'payment_information' => 'Payment Information',
        'billing_address' => 'Billing Address',
        'order_items' => 'Order Items',
        'order_actions' => 'Order Actions',

        // Customer info
        'name' => 'Name',
        'email' => 'Email',
        'phone' => 'Phone',
        'na' => 'N/A',

        // Payment info
        'payment_method' => 'Payment Method',
        'payment_methods' => [
            'cod' => 'Cash on Delivery',
            'paypal' => 'PayPal',
            'stripe' => 'Stripe (Credit Card)',
        ],
        'payment_status' => 'Payment Status',
        'payment_statuses' => [
            'paid' => 'Paid',
            'unpaid' => 'Unpaid',
            'pending' => 'Pending',
        ],
        'transaction_id' => 'Transaction ID',

        // Order items table
        'product' => 'Product',
        'quantity' => 'Quantity',
        'unit_price' => 'Unit Price',
        'subtotal' => 'Subtotal',
        'product_not_available' => 'Product no longer available',
        'no_items' => 'No items found in this order.',
        'total' => 'Total',

        // Order status
        'update_order_status' => 'Update Order Status',
        'update_status' => 'Update Status',
        'order_statuses' => [
            'pending' => 'Pending',
            'processing' => 'Processing',
            'completed' => 'Completed',
            'cancelled' => 'Cancelled',
        ],
    ],

    // Newsletter Management
    'newsletter' => [
        'title' => 'Newsletter Subscribers',
        'manage_subscribers' => 'Manage your email subscribers',
        'send_newsletter' => 'Send Newsletter',
        'export_csv' => 'Export to CSV',
        'compose' => 'Compose Newsletter',
        'back_to_subscribers' => 'Back to Subscribers',

        // Stats
        'stats' => [
            'total_subscribers' => 'Total Subscribers',
            'active' => 'Active',
            'unsubscribed' => 'Unsubscribed',
        ],

        // Filters
        'search_email' => 'Search Email',
        'search_placeholder' => 'Search by email...',
        'status' => 'Status',
        'all_statuses' => 'All Statuses',
        'status_active' => 'Active',
        'status_unsubscribed' => 'Unsubscribed',
        'filter' => 'Filter',
        'reset' => 'Reset',

        // Table
        'table' => [
            'id' => 'ID',
            'email' => 'Email',
            'status' => 'Status',
            'subscribed_date' => 'Subscribed Date',
            'actions' => 'Actions',
        ],

        // Actions
        'toggle_status' => 'Toggle Status',
        'unsubscribe' => 'Unsubscribe',
        'reactivate' => 'Reactivate',
        'delete' => 'Delete',
        'delete_confirm' => 'Are you sure you want to delete this subscriber?',

        // Empty state
        'no_subscribers' => 'No subscribers found',

        // Compose form
        'compose_newsletter' => 'Compose Newsletter',
        'email_subject' => 'Email Subject',
        'subject_placeholder' => 'Enter email subject...',
        'email_content' => 'Email Content',
        'content_placeholder' => 'Write your newsletter content here...',
        'preview' => 'Preview',
        'recipients_info' => 'This newsletter will be sent to :count active subscriber(s)',
        'send_newsletter_button' => 'Send Newsletter',
        'send_confirm' => 'Are you sure you want to send this newsletter to all active subscribers?',

        // Messages
        'success_sent' => 'Newsletter queued successfully! Sending to :count subscriber(s).',
        'error_no_subscribers' => 'No active subscribers to send emails to.',
        'success_status_updated' => 'Subscriber status updated successfully.',
        'success_deleted' => 'Subscriber deleted successfully.',
        'success_exported' => 'Subscribers exported successfully.',
    ],

];
