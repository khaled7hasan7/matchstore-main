...
<div class="auth-container">
    <div class="row g-0 min-vh-100">
        ...
        <div class="col-lg-6 auth-branding d-flex flex-column justify-content-center align-items-center text-white px-5 py-5"
            ...
            {{-- Welcome Message --}}
            <h2 class="fw-bold mb-3 text-center">{{ __('store.login.welcome_back') }}</h2>
        ...
        <div class="col-lg-6 auth-form-section d-flex flex-column justify-content-center align-items-center bg-white px-5 py-5">
            ...
            <div class="auth-form-container" style="max-width: 450px; width: 100%;">
                ...
                <div class="text-center mb-4">
                    <h3 class="fw-bold mb-2">{{ __('store.login.login_now') }}</h3>
                    <p class="text-muted">{{ __('store.login.form_subtitle') }}</p>
                </div>
                ...
                <form method="POST" action="{{ route('customer.login') }}">
                    ...
                    <div class="text-center">
                        <span class="text-muted">{{ __('store.login.dont_have_account') }}</span>
                        <a href="{{ route('customer.register') }}"
                           class="text-decoration-none fw-semibold ms-1"
                           style="color: {{ config('store.accent_color', '#ffc107') }};">
                            {{ __('store.login.signup') }}
                        </a>
                    </div>
                </form>
            </div>
...
<div class="mobile-only mobile-auth-page">
    ...
    <div class="mobile-auth-header">
        ...
        <h1>{{ __('store.login.welcome_back') }}</h1>
        <p>{{ __('store.login.login_description') }}</p>
    </div>

    {{-- Mobile Auth Form --}}
    <div class="mobile-auth-form-wrapper">
        <div class="mobile-auth-form">
            <h2 class="mobile-auth-form-title">{{ __('store.login.login_now') }}</h2>
            <p class="mobile-auth-form-subtitle">{{ __('store.login.form_subtitle') }}</p>

            <form method="POST" action="{{ route('customer.login') }}">
                @csrf

                <div class="mobile-form-group">
                    <label class="mobile-form-label">{{ __('store.login.email') }}</label>
                    <input type="email"
                           name="email"
                           value="{{ old('email') }}"
                ...
                <div class="mobile-form-group">
                    <label class="mobile-form-label">{{ __('store.login.password') }}</label>
                    <input type="password"
                           name="password"
                ...
                <div class="mobile-remember-row">
                    <div class="mobile-checkbox-wrap">
                        <input type="checkbox" name="remember" id="mobile-remember">
                        <label for="mobile-remember">{{ __('store.login.remember_me') }}</label>
                    </div>
                    <a href="{{ route('customer.password.request') }}" class="mobile-forgot-link">
                        {{ __('store.login.forgot_password') }}
                    </a>
                </div>
                ...
                <div class="mobile-auth-footer">
                    <span>{{ __('store.login.dont_have_account') }}</span>
...
<style>
    ...
    .mobile-checkbox-wrap input[type="checkbox"] {
        width: 18px;
        height: 18px;
        accent-color: var(--main-color);
    }

    .mobile-checkbox-wrap label {
        font-size: 13px;
        color: #495057;
    }
